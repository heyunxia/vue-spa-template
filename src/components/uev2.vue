                ...
                window.UE.registerUI('skip',function(editor,uiName){
                    ...
                    this.ueditor.execCommand('inserthtml', inHtml);
                      ...
                      //注册按钮执行时的command命令，使用命令默认就会带有回退操作
                         editor.registerCommand(uiName, {
                             execCommand: function() {
                                const inHtml = `<div><p style="padding-bottom: 10px;"> </p><p><video controls  poster="${this.videoUpload.mainPath}"  src="${this.link}" style="width: 100%;height: 200px;object-fit: fill"></video></p></div><p style="padding-bottom: 10px;"> </p>`;
                                return inHtml;
                             }
                         });
                         //创建一个button
